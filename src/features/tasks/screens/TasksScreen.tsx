...
export function TasksScreen() {
  ...
  const visibleTasks = useMemo(() => {
    ...
    return allTasks.filter(task => {
      ...
      if (filter === 'upcoming')
        return startOfDay(taskDate).getTime() > today.getTime();
      return true;
    });
  }, [allTasks, filter]);
  ...
  const taskToDelete = useMemo(
    () =>
      taskToDeleteId
        ? allTasks.find(task => task.id === taskToDeleteId) ?? null
        : null,
  ...
  const handleToggleFromDetails = useCallback(
    (task: Task) => {
      if (busy) return;
      dispatch(toggleTask(task.id));
    },
  ...
  return (
    <View style={styles.safeArea}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <TaskEmptyState disabled={busy} onCreate={handleCreateTask} />
        }
