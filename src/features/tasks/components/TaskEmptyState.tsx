...
export function TaskEmptyState({disabled = false, onCreate}: TaskEmptyStateProps) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.iconContainer}>
        <FontAwesome6 name="check" size={23} color={theme.colors.success} iconStyle="solid" />
      </View>
      <Text style={styles.title}>No tasks here</Text>
      <Text style={styles.text}>You're all caught up.{"\n"}Enjoy the moment.</Text>
      <Pressable
        onPress={onCreate}
        disabled={disabled}
        style={({pressed}) => [
